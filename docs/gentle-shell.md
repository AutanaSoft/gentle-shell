...
The source checkout currently prepares `gentle-pi` `3.6.0` with a package-local Gentle AI `v3.6.0` pin; this is not a claim that `3.6.0` is published.
...
- `subagent_run.workspace_root` selects an existing linked worktree in the parent's Git clone only. Validation happens before queueing; the child runs at that canonical root. Successful OS spawn registers it in the originating parent's same-clone registry, including delayed queued launches; failed spawns do not register.
